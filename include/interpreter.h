...
extern char *Object2CString_r(JHandle *, char *, int);
...
#define obj_monitor(handlep) ((unsigned int) handlep)
/*
 * We need a second monitor for class objects so method synchronization doesn't
 * interfer with internal interpreter and compiler processing of the class
 *
 * The correction macro must be used if the second monitor adddress
 * is to be recognized by ValidHandle (in the dump routine).
 */
#define int_class_monitor(handlep) (1 + obj_monitor(handlep))
#define int_class_correct(x) (((unsigned int)(x)) - 1)
