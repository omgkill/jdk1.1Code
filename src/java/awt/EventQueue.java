		...
		if (q.id == eqi.id) {
		    switch (q.id) {
			  ...
			  break;
                ...
                } else if ((eqi.id == PaintEvent.UPDATE) &&
                    (q.id == PaintEvent.PAINT)) {
                    // we are putting UPDATE on queue which allready has PAINT
                    // This case was added to remove extra repaints on startup
                    // and on resizing frames.
                    PaintEvent pe = (PaintEvent)q.event;
                    if (pe.getSource() == theEvent.getSource()) {
                        Rectangle rect = pe.getUpdateRect();
                        Rectangle newRect =
                              ((PaintEvent)theEvent).getUpdateRect();
                        // only coalesce if UPDATE completly contains the PAINT
                        if (newRect.contains(rect.x,rect.y) && newRect.contains(
                               rect.x + rect.width,rect.y + rect.height)) {
                            pe.id = PaintEvent.UPDATE;
                            pe.setUpdateRect(newRect);
                            return;
                        }
                    }    
