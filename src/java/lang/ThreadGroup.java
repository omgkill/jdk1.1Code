/*
 * @(#)ThreadGroup.java	1.38 01/12/12
 *
...
class ThreadGroup {
   ...
   // #4148446
   // There is a narrow timing windows in ThreadRT0/threadFree between
   // setting Thread.stillborn and performing the "Thread.exit" Java callback.
   // Thread.exit, in turn, will call group.remove(Thread) to rid any groups
   // of the dead thread.  In the window, if another thread, T1, calls Thread.join
   // on the moribund thread, join will return immediately indicating that
   // the thread is dead.  If T1 then calls ThreadGroup.destroy it will get
   // an unexpected "IllegalThreadState" exception as the dying thread still
   // resides in the group.threads[] list. 
   // 
   // There are two ways to handle the problem:
   // (1) Perform the Thread.exit callback _before_ setting Thread.stillborn.
   // -OR-
   // (2) In ThreadGroup.destroy perform the cleanup if we encounter moribund 
   //  	  threads.  I've opted for this approach. 
   // 
    
    private final int HarvestMoribundThreads ()
    {
	int zombies = 0 ; 
	
	for (int i = 0 ; i < nthreads ; i++ ) {
		if (!threads[i].isAlive()) {	// check Thread.stillborn flag
			remove (threads[i]) ; 
			i = 0 ;			// restart loop 
			++zombies ;
		}
